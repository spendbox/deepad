...
import { adminCheckPaystack, adminRecleanMessages } from '../../../actions';
...
export function RecleanButton({ eventId }: { eventId: string }) {
  const [state, action, pending] = useActionState(adminRecleanMessages.bind(null, eventId), null);
  return (
    <form action={action} className="actions">
      <button type="submit" className="btn btn-sm" disabled={pending}>{pending ? 'Fixing…' : 'Re-check all messages'}</button>
      {state?.ok && <span className="ok-text" role="status">{state.ok}</span>}
      {state?.error && <span className="error-text" role="alert">{state.error}</span>}
    </form>
  );
